...
contract ERC165RevertInvalidUpgradeable is Initializable, SupportsInterfaceWithLookupMockUpgradeable {
    function __ERC165RevertInvalid_init(bytes4[] memory interfaceIds) internal onlyInitializing {
        __SupportsInterfaceWithLookupMock_init_unchained();
        __ERC165RevertInvalid_init_unchained(interfaceIds);
    }

    function __ERC165RevertInvalid_init_unchained(bytes4[] memory interfaceIds) internal onlyInitializing {
        for (uint256 i = 0; i < interfaceIds.length; i++) {
            _registerInterface(interfaceIds[i]);
        }
    }
